...
from pandas.tseries.offsets import BDay  # 导入交易日工具
...
def calculate_performance_metrics(actual_df, pred_df):
    ...
    try:
        ...
        ic = np.corrcoef(actual_close, pred_close)[0, 1] if len(actual_close) > 1 else float('nan')
    ...
    if emotion_df is not None:
        try:
            ...
            if emotion_data.empty:
                ...
                # 填充可能的缺失值并确保索引对齐
                emotion_data = emotion_data.reindex(emotion_dates_dt, fill_value=0.0)
            ...
            if y_emotion_data.empty:
                ...
                # 填充可能的缺失值并确保索引对齐
                y_emotion_data = y_emotion_data.reindex(y_emotion_dates_dt, fill_value=0.0)

        except KeyError as e:
            print(f"Warning: Some dates not found in emotion data: {e}")
            emotion_data = None
            y_emotion_data = None
        except Exception as e:
...
def create_comparison_dataframe(actual_df, pred_df):
    """
    创建包含实际收盘价和预测价格的DataFrame

    Parameters:
    actual_df: pandas.DataFrame, 包含实际值的DataFrame
    pred_df: pandas.DataFrame, 包含预测值的DataFrame

    Returns:
    comparison_df: pandas.DataFrame, 包含实际收盘价和预测价格的对比DataFrame
    """
    # 确保两个DataFrame都有相同的索引
    common_index = actual_df.index.intersection(pred_df.index)
    ...
    if len(common_index) == 0:
        ...
        min_len = min(len(actual_df), len(pred_df))
        if min_len == 0:
            return pd.DataFrame(columns=['Close', 'pred_price'])

        # 取前min_len行进行比较
        actual_subset = actual_df.iloc[:min_len]
        pred_subset = pred_df.iloc[:min_len]

        # 创建具有datetime索引的DataFrame
        comparison_df = pd.DataFrame(index=actual_subset.index)
        comparison_df['Close'] = actual_subset['close'].values
        comparison_df['pred_price'] = pred_subset['close'].values
    else:
        actual_subset = actual_df.loc[common_index]
        pred_subset = pred_df.loc[common_index]

        # 创建对比DataFrame
        comparison_df = pd.DataFrame(index=common_index)
        comparison_df['Close'] = actual_subset['close']
        comparison_df['pred_price'] = pred_subset['close']
...
def plot_prediction_results(kline_df, pred_df, actual_y_df, actual_y_timestamp, stock_name, output_dir, metrics=None):
    ...
    # 添加性能指标到标题
    title = f'{stock_name} - Close Price Prediction (单Kronos)'
    if metrics:
        valid_metrics = {k: v for k, v in metrics.items() if not np.isnan(v)}
        if valid_metrics:
            metric_text = " | ".join([f"{k}: {v:.4f}" for k, v in valid_metrics.items() if k in ['MSE', 'MAE', 'RMSE', 'IC', 'RankIC']])
            title += f'\n{metric_text}'
...
def plot_overall_performance(metrics_dict, output_dir):
    ...
    if len(numeric_columns) > 0:
        ...
        # 绘制平均性能指标
        bars = plt.bar(avg_metrics.index, avg_metrics.values, color=['skyblue', 'lightgreen', 'lightcoral', 'gold', 'plum'])
...
def main():
    ...
    model_path = "../finetune/outputs/models/finetune_predictor_demo/checkpoints/best_model"
    # tokenizer_path = "../finetune/outputs/models/finetune_tokenizer_demo/checkpoints/best_tokenizer"
    output_dir = "./kronos_prediction_results"
    ...
    pred_len = 7       # 预测7天
    pred_start_idx = None # 颐测起始索引，None表示使用最新data
    ...
    model = Kronos.from_pretrained(model_path).to(device)  # 模型移到指定设备
    ...
    for i, kline_file in enumerate(stock_files):
        ...
        try:
            ...
            x_df, x_timestamp, y_timestamp, emotion_data, y_emotion_data, actual_y_df, actual_y_timestamp = prepare_data_for_prediction(
                kline_df, None, lookback_window, pred_len, pred_start_idx,
            )
            print(x_df)
            if x_df is None:
            ...
            if isinstance(pred_result, torch.Tensor):
                # 先移到CPU -> 脛离计算图 -> 轻为numpy -> 构建DataFrame
            ...
            # 创建并保存实际收盘价与预测价格的对比DataFrame
            if not actual_y_df.empty and not pred_df.empty:
                comparison_df = create_comparison_dataframe(actual_y_df, pred_df)
                if not comparison_df.empty:
                    comparison_output_file = os.path.join(output_dir, f"{stock_name}_comparison.csv")
                    comparison_df.to_csv(comparison_output_file)
                    print(f"Comparison data saved to {comparison_output_file}")
